...
static int
get_status (RM_RebootStatus *status, RM_RebootMethod *method)
{
  struct p {
...
print_full_status (void)
{
  ...
  static const sd_json_dispatch_field dispatch_table[] = {
    { "RebootStatus",              SD_JSON_VARIANT_INTEGER, sd_json_dispatch_int, offsetof(struct p, status), SD_JSON_MANDATORY },
    { "RequestedMethod",           SD_JSON_VARIANT_INTEGER, sd_json_dispatch_int, offsetof(struct p, method),                 0 },
    { "RebootStrategy",            SD_JSON_VARIANT_INTEGER, sd_json_dispatch_int, offsetof(struct p, strategy),               0 },
    { "MaintenanceWindowStart",    SD_JSON_VARIANT_STRING,  sd_json_dispatch_string, offsetof(struct p, maint_window_start),  0 },
    { "MaintenanceWindowDuration", SD_JSON_VARIANT_INTEGER, sd_json_dispatch_int, offsetof(struct p, maint_window_duration),  0 },
    {}
  };
  _cleanup_(sd_varlink_unrefp) sd_varlink *link = NULL;
  sd_json_variant *result;
  ...
  r = rm_status_to_str (p.status, p.method, &str);
  if (r < 0)
    {
  ...
  r = rm_strategy_to_str (p.strategy, &str);
  if (r < 0)
    {
  ...
  if (p.maint_window_start)
    {
      printf ("Start of maintenance window: %s\n", p.maint_window_start);
      printf ("Duration of maintenance window: %s\n", duration_to_string (p.maint_window_duration));
    }
...
main (int argc, char **argv)
{
  ...
  if (strcasecmp ("reboot", argv[1]) == 0)
  ...
  else if (strcasecmp ("status", argv[1]) == 0)
    {
      int quiet = 0;
      int full = 0;
      RM_RebootStatus status = 0;
      RM_RebootMethod method = 0;
	...
	{
	  int r = get_status (&status, &method);
	  if (r < 0)
	    retval = 1;
	  else if (quiet)
	    retval = status;
	      ...
	      if (status >= 0)
		{
		  ...
		  r = rm_status_to_str (status, method, &str);
		  if (r < 0)
		    {
		      fprintf (stderr, "Converting status to string failed: %s\n",
			       strerror (-r));
  ...
  else if (strcasecmp ("get-strategy", argv[1]) == 0)
    {
      ...
      // XXX strategy = get_strategy (connection);
      if (strategy < 0)
	retval = 1;
      else
	{
